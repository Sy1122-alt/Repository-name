...
(function(){
  ...
  function fracIt(x){
    ...
    // unicode 上下标转 KaTeX（组合成花括号上标，防渲染报错红显）：3ⁿ⁺¹ → 3^{n+1}
    var sup={'⁰':'0','¹':'1','²':'2','³':'3','⁴':'4','⁵':'5','⁶':'6','⁷':'7','⁸':'8','⁹':'9','ⁿ':'n','ⁱ':'i','⁺':'+','⁻':'-'};
    x=x.replace(/([0-9a-zA-Z\)\]\}])[⁰¹²³⁴⁵⁶⁷⁸⁹ⁿⁱ⁺⁻]+/g, function(m,base){
      return base+'^{'+m.slice(1).replace(/[⁰¹²³⁴⁵⁶⁷⁸⁹ⁿⁱ⁺⁻]/g,function(c){return sup[c];})+'}';
    });
    var sub={'₀':'0','₁':'1','₂':'2','₃':'3','₄':'4','₅':'5','₆':'6','₇':'7','₈':'8','₉':'9','ₙ':'n','ₓ':'x'};
    x=x.replace(/([0-9a-zA-Z\)\]\}])[₀₁₂₃₄₅₆₇₈₉ₙₓ]+/g, function(m,base){
      return base+'_{'+m.slice(1).replace(/[₀₁₂₃₄₅₆₇₈₉ₙₓ]/g,function(c){return sub[c];})+'}';
    });
    // 根号：√(x) → \sqrt{x}；√x → \sqrt{x}
    x=x.replace(/√\(([^()]*)\)/g,"\\sqrt{$1}").replace(/√([0-9a-zA-Z]+)/g,"\\sqrt{$1}");
    // 圈数字转 (数字)，防 KaTeX 不支持红显（①→(1)）
    x=x.replace(/[①-⑨]/g,function(c){return '('+(c.charCodeAt(0)-0x2460+1)+')';}).replace(/⑩/g,"(10)");
  ...
  function show(){
    ...
    box.innerHTML=modeHint+
      '<div class="card">'
      +'<div class="meta">'+meta+'<span style="margin-left:auto;display:flex;gap:6px;"><button id="btnDel" style="background:#EA6668;color:#fff;border:none;border-radius:6px;padding:2px 10px;font-size:12px;cursor:pointer;">删除此题</button></span></div>'
...
@media (max-width:480px){
  ...
  .similar a{min-height:40px;padding:8px 10px;}
  .qtext{font-size:16px;}
}
</style>
__KATEX_CSS__
...
(function(){
  ...
  function findSimilar(it0){
    var t0=it0["专题"]||"", k0=it0["考点"]||"";
    var out=[], seen={};
    function push(c){ if(!seen[c.id]){ seen[c.id]=1; out.push(c); } }
    ALL.forEach(function(c){ if(c.id!==it0.id && c["专题"]===t0 && c["考点"]===k0) push(c); });
    ALL.forEach(function(c){ if(c.id!==it0.id && c["专题"]===t0) push(c); });
    if(!out.length) ALL.forEach(function(c){ if(c.id!==it0.id && c["题型"]===it0["题型"]) push(c); });
    return out.slice(0,3);
  }
  function pool(){
  ...
  function show(){
    ...
    html+='</div>';
    var similar=findSimilar(it);
    html+='<div class="similar"><div>再来一道同专题题</div>'
      +(similar.length?similar.map(function(c,n){return '<a href="#" data-similar="'+n+'">'+renderMath(c.id+' · '+(c["题目"]||"未命名"))+'</a>';}).join(""):'<span>暂无同专题题。</span>')
      +'</div>';
    ...
    box.querySelectorAll("a[data-similar]").forEach(function(link){
      link.addEventListener("click",function(e){
        e.preventDefault();
        var cand=similar[Number(link.getAttribute("data-similar"))];
        if(!cand) return;
        list=[cand].concat(list.filter(function(x){return x.id!==cand.id;}));
        idx=0; revealed=false; show();
      });
    });
    // 英语刷题：点击单词加入生词本
